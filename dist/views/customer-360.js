...
export const customer360 = sqliteView('customer_360').as((qb) => {
    return qb
        .select({
        // Customer info
        customerId: sql `c.customer_id`.as('customer_id'),
        customerName: sql `c.name`.as('customer_name'),
        customerCode: sql `c.customer_code`.as('customer_code'),
        email: sql `c.email`.as('email'),
        phone: sql `c.phone`.as('phone'),
        isActive: sql `c.is_active`.as('is_active'),
        // Pricing and terms
        pricingSchemeId: sql `c.pricing_scheme_id`.as('pricing_scheme_id'),
        pricingSchemeName: sql `ps.name`.as('pricing_scheme_name'),
        paymentTermsId: sql `c.payment_terms_id`.as('payment_terms_id'),
        paymentTermsName: sql `pt.name`.as('payment_terms_name'),
        // Order metrics (all time)
        totalOrders: sql `COALESCE(order_stats.total_orders, 0)`.as('total_orders'),
        completedOrders: sql `COALESCE(order_stats.completed_orders, 0)`.as('completed_orders'),
        cancelledOrders: sql `COALESCE(order_stats.cancelled_orders, 0)`.as('cancelled_orders'),
        // Revenue metrics
        totalRevenue: sql `COALESCE(order_stats.total_revenue, '0')`.as('total_revenue'),
      ...
      `.as('average_order_value'),
        // Open order metrics
        openOrders: sql `COALESCE(order_stats.open_orders, 0)`.as('open_orders'),
        openOrdersValue: sql `COALESCE(order_stats.open_orders_value, '0')`.as('open_orders_value'),
        // Dates
        firstOrderDate: sql `order_stats.first_order_date`.as('first_order_date'),
        lastOrderDate: sql `order_stats.last_order_date`.as('last_order_date'),
    })
      ...
      ) order_stats`, sql `c.customer_id = order_stats.customer_id`);
});
/**
...
`;
//# sourceMappingURL=customer-360.js.map