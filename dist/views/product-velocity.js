...
export const productVelocity = sqliteView('product_velocity').as((qb) => {
    return qb.select({
        ...
        categoryName: sql `category_name`.as('category_name'),
        quantityOnHand: sql `quantity_on_hand`.as('quantity_on_hand'),
        sold7d: sql `sold_7d`.as('sold_7d'),
        sold30d: sql `sold_30d`.as('sold_30d'),
        sold90d: sql `sold_90d`.as('sold_90d'),
        avgDailySales: sql `avg_daily_sales`.as('avg_daily_sales'),
        daysOfStock: sql `days_of_stock`.as('days_of_stock'),
        lastSaleDate: sql `last_sale_date`.as('last_sale_date'),
        velocityTier: sql `velocity_tier`.as('velocity_tier'),
    }).from(sql `(
    SELECT
      p.product_id,
      p.sku,
...
WHERE p.is_active = 1;
`;
//# sourceMappingURL=product-velocity.js.map