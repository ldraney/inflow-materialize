...
export const stockMovementLedger = sqliteView('stock_movement_ledger').as((qb) => {
    ...
    return qb
        .select({
        movementType: sql `'PLACEHOLDER'`.as('movement_type'),
        movementDate: sql `NULL`.as('movement_date'),
        productId: sql `NULL`.as('product_id'),
        sku: sql `NULL`.as('sku'),
        productName: sql `NULL`.as('product_name'),
        locationId: sql `NULL`.as('location_id'),
        locationName: sql `NULL`.as('location_name'),
        quantity: sql `NULL`.as('quantity'),
        referenceType: sql `NULL`.as('reference_type'),
        referenceNumber: sql `NULL`.as('reference_number'),
        referenceId: sql `NULL`.as('reference_id'),
        lineId: sql `NULL`.as('line_id'),
    })
...
`;
//# sourceMappingURL=stock-movement-ledger.js.map