...
export const deadStock = sqliteView('dead_stock').as((qb) => {
    return qb.select({
        ...
        unitCost: sql `unit_cost`.as('unit_cost'),
        totalValue: sql `total_value`.as('total_value'),
        lastMovementDate: sql `last_movement_date`.as('last_movement_date'),
        daysSinceMovement: sql `days_since_movement`.as('days_since_movement'),
        deadStockTier: sql `dead_stock_tier`.as('dead_stock_tier'),
    }).from(sql `(
    SELECT
      inv.product_id,
      inv.sku,
...
WHERE julianday('now') - julianday(COALESCE(movements.last_movement_date, '1900-01-01')) >= 30;
`;
//# sourceMappingURL=dead-stock.js.map