...
export const inventoryDetail = sqliteView('inventory_detail').as((qb) => {
    return qb
        ...
        trackSerials: sql `p.track_serials`.as('track_serials'),
        trackLots: sql `p.track_lots`.as('track_lots'),
        trackExpiry: sql `p.track_expiry`.as('track_expiry'),
        shelfLifeDays: sql `p.shelf_life_days`.as('shelf_life_days'),
    })
