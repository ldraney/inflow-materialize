...
export const transferPipeline = sqliteView('transfer_pipeline').as((qb) => {
    return qb
        .select({
        // Transfer header
        stockTransferId: sql `st.stock_transfer_id`.as('stock_transfer_id'),
        transferNumber: sql `st.transfer_number`.as('transfer_number'),
        status: sql `st.status`.as('status'),
        transferDate: sql `st.transfer_date`.as('transfer_date'),
        remarks: sql `st.remarks`.as('remarks'),
        lastModified: sql `st.last_modified_date_time`.as('last_modified'),
        // From location
        fromLocationId: sql `st.from_location_id`.as('from_location_id'),
        fromLocationName: sql `from_loc.name`.as('from_location_name'),
        fromLocationAbbreviation: sql `from_loc.abbreviation`.as('from_location_abbreviation'),
        // To location
        toLocationId: sql `st.to_location_id`.as('to_location_id'),
        toLocationName: sql `to_loc.name`.as('to_location_name'),
        toLocationAbbreviation: sql `to_loc.abbreviation`.as('to_location_abbreviation'),
        // Line item details
        stockTransferLineId: sql `stl.stock_transfer_line_id`.as('stock_transfer_line_id'),
        lineNum: sql `stl.line_num`.as('line_num'),
        productId: sql `stl.product_id`.as('product_id'),
        sku: sql `p.sku`.as('sku'),
        productName: sql `p.name`.as('product_name'),
        quantity: sql `stl.quantity`.as('quantity'),
        fromSublocation: sql `stl.from_sublocation`.as('from_sublocation'),
        toSublocation: sql `stl.to_sublocation`.as('to_sublocation'),
    })
...
`;
//# sourceMappingURL=transfer-pipeline.js.map