...
export const categoryInventorySummary = sqliteView('category_inventory_summary').as((qb) => {
  return qb
    .select({
      // Category info
      categoryId: sql<string>`c.category_id`.as('category_id'),
      categoryName: sql<string>`c.name`.as('category_name'),
      parentCategoryId: sql<string>`c.parent_category_id`.as('parent_category_id'),
      parentCategoryName: sql<string>`pc.name`.as('parent_category_name'),

      // Product counts
      skuCount: sql<number>`COUNT(DISTINCT p.product_id)`.as('sku_count'),
      activeSkuCount: sql<number>`SUM(CASE WHEN p.is_active = 1 THEN 1 ELSE 0 END)`.as('active_sku_count'),

      // Quantity metrics (global, not location-specific)
      totalQuantityOnHand: sql<number>`SUM(CAST(COALESCE(ps.quantity_on_hand, '0') AS REAL))`.as('total_quantity_on_hand'),
      totalQuantityAvailable: sql<number>`SUM(CAST(COALESCE(ps.quantity_available, '0') AS REAL))`.as('total_quantity_available'),
      totalQuantityReserved: sql<number>`SUM(CAST(COALESCE(ps.quantity_reserved, '0') AS REAL))`.as('total_quantity_reserved'),
      totalQuantityOnPurchaseOrder: sql<number>`SUM(CAST(COALESCE(ps.quantity_on_purchase_order, '0') AS REAL))`.as('total_quantity_on_purchase_order'),
      totalQuantityOnSalesOrder: sql<number>`SUM(CAST(COALESCE(ps.quantity_on_sales_order, '0') AS REAL))`.as('total_quantity_on_sales_order'),

      // Value metrics (using average cost from product_summary)
      totalStockValue: sql<string>`
        CAST(SUM(
