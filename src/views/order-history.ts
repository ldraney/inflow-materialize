...
export const orderHistory = sqliteView('order_history').as((qb) => {
  return qb.select({
    ...
    partnerId: sql<string>`partner_id`.as('partner_id'),
    partnerName: sql<string>`partner_name`.as('partner_name'),
    locationId: sql<string>`location_id`.as('location_id'),
    locationName: sql<string>`location_name`.as('location_name'),
    lineId: sql<string>`line_id`.as('line_id'),
    ...
    quantityOrdered: sql<string>`quantity_ordered`.as('quantity_ordered'),
    quantityFulfilled: sql<string>`quantity_fulfilled`.as('quantity_fulfilled'),
    unitPrice: sql<string>`unit_price`.as('unit_price'),
    lineTotal: sql<string>`line_total`.as('line_total'),
  }).from(sql`(
