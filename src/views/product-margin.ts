...
export const productMargin = sqliteView('product_margin').as((qb) => {
  return qb
    .select({
      ...
      defaultSellPrice: sql<string>`pp.price`.as('default_sell_price'),
      pricingSchemeId: sql<string>`pp.pricing_scheme_id`.as('pricing_scheme_id'),
      pricingSchemeName: sql<string>`ps.name`.as('pricing_scheme_name'),

      // Vendor cost (from preferred vendor or lowest cost vendor)
      vendorCost: sql<string>`vendor_cost.cost`.as('vendor_cost'),
      preferredVendorId: sql<string>`vendor_cost.vendor_id`.as('preferred_vendor_id'),
      preferredVendorName: sql<string>`v.name`.as('preferred_vendor_name'),

      // Margin calculations
      marginAmount: sql<string>`
        CAST(COALESCE(CAST(pp.price AS REAL), 0) - COALESCE(CAST(vendor_cost.cost AS REAL), 0) AS TEXT)
      `.as('margin_amount'),

      marginPercent: sql<string>`
        CASE
    ...
    .leftJoin(
      sql`product_prices pp`,
      sql`p.product_id = pp.product_id`
    )
    ...
    .leftJoin(
      ...
      sql`p.product_id = vendor_cost.product_id AND vendor_cost.rn = 1`
    )
