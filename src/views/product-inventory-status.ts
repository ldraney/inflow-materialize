...
export const productInventoryStatus = sqliteView('product_inventory_status').as((qb) => {
  return qb
    .select({
      ...
      name: sql<string>`p.name`.as('name'),
      description: sql<string>`p.description`.as('description'),
      ...
      quantityBuildable: sql<string>`ps.quantity_buildable`.as('quantity_buildable'),
