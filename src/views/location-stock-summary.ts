...
export const locationStockSummary = sqliteView('location_stock_summary').as((qb) => {
  return qb
    .select({
      locationId: sql<string>`l.location_id`.as('location_id'),
      ...
      skuCount: sql<number>`COUNT(DISTINCT ps.product_id)`.as('sku_count'),
      ...
      totalQuantityInTransit: sql<number>`SUM(CAST(COALESCE(ps.quantity_in_transit, '0') AS REAL))`.as('total_quantity_in_transit'),
    })
