...
export const locationReorderAlerts = sqliteView('location_reorder_alerts').as((qb) => {
  return qb
    .select({
      // Product info
      productId: sql<string>`p.product_id`.as('product_id'),
      ...
      // Location info
      locationId: sql<string>`rs.location_id`.as('location_id'),
      ...
      reorderPoint: sql<string>`rs.reorder_point`.as('reorder_point'),
      reorderQuantity: sql<string>`rs.reorder_quantity`.as('reorder_quantity'),

      // Shortfall calculation
      shortfallQuantity: sql<number>`
        CAST(rs.reorder_point AS REAL) -
        CAST(COALESCE(ps.quantity_available, '0') AS REAL)
      `.as('shortfall_quantity'),

      // Suggested order quantity
      suggestedOrderQuantity: sql<number>`
      ...
      // Preferred vendor
      preferredVendorId: sql<string>`rs.vendor_id`.as('preferred_vendor_id'),
      vendorName: sql<string>`v.name`.as('vendor_name'),
      vendorCode: sql<string>`v.vendor_code`.as('vendor_code'),

      // Vendor item details
      vendorItemCode: sql<string>`vi.vendor_item_code`.as('vendor_item_code'),
      vendorCost: sql<string>`vi.cost`.as('vendor_cost'),
      leadTimeDays: sql<number>`vi.lead_time_days`.as('lead_time_days'),

      // Transfer source (if configured)
      fromLocationId: sql<string>`rs.from_location_id`.as('from_location_id'),
    })
    .from(sql`products p`)
