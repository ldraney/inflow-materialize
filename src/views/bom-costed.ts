...
export const bomCosted = sqliteView('bom_costed').as((qb) => {
  return qb
    .select({
      // BOM line info
      itemBomId: sql<string>`bom.item_bom_id`.as('item_bom_id'),

      // Parent product (the assembled product)
      parentProductId: sql<string>`bom.parent_product_id`.as('parent_product_id'),
      parentSku: sql<string>`parent.sku`.as('parent_sku'),
      parentProductName: sql<string>`parent.name`.as('parent_product_name'),

      // Component product
      componentProductId: sql<string>`bom.component_product_id`.as('component_product_id'),
      componentSku: sql<string>`component.sku`.as('component_sku'),
      componentProductName: sql<string>`component.name`.as('component_product_name'),

      // BOM quantity
      quantity: sql<string>`bom.quantity`.as('quantity'),

      // Component cost (from preferred vendor or lowest cost)
      componentUnitCost: sql<string>`COALESCE(component_cost.cost, '0')`.as('component_unit_cost'),
      componentVendorId: sql<string>`component_cost.vendor_id`.as('component_vendor_id'),
      componentVendorName: sql<string>`v.name`.as('component_vendor_name'),
      ...
      totalBomCost: sql<string>`bom_totals.total_bom_cost`.as('total_bom_cost'),
      componentCount: sql<number>`bom_totals.component_count`.as('component_count'),
    })
    ...
    .leftJoin(
      ...
      sql`bom.component_product_id = component_cost.product_id AND component_cost.rn = 1`
    )
    ...
    .leftJoin(
      ...
      sql`bom.parent_product_id = bom_totals.parent_product_id`
    );
});
